...
建议使用紧凑的 C 结构体表示数据包，采用小端字节序：

```cpp
#pragma pack(push, 1)
struct DockingPacket {
    uint64_t timestamp;      // 0  : 8 字节，系统时间戳 (ns)
    float    distance_m;     // 8  : 4 字节，垂直距离
    float    angle_deg;      // 12 : 4 字节，偏角
    uint8_t  confidence;     // 16 : 1 字节，0-100
    uint8_t  status;         // 17 : 1 字节，0:未检测, 1:正常, 2:警告/无效

    uint8_t  sensor_id;      // 18 : 1 字节，雷达编号
    uint8_t  reserved;       // 19 : 1 字节，对齐或后续扩展

    float    sensor_x_m;     // 20 : 4 字节，雷达在船体系下位置与朝向
    float    sensor_y_m;     // 24 : 4 字节
    float    sensor_z_m;     // 28 : 4 字节
    float    sensor_yaw_deg; // 32 : 4 字节
};
#pragma pack(pop)
```
...
- `distance_m`：
  - 船体参考点到当前目标岸线的垂直距离（单位：米）；
  - 在船体坐标系 B 下计算而得。

- `angle_deg`：
  - 船首方向与岸线方向的夹角（度）；
  - 正负号与旋转方向约定需在控制系统与本系统一致。

- `confidence`：
  - 岸线检测置信度，0-100；
  - 可根据 RANSAC 内点数量、残差等归一化。

- `status`：
  - `0`：未检测（例如当前无足够点云或处于初始化阶段）；
  - `1`：正常（输出可直接用于控制）；
  - `2`：警告/无效（例如超时未检测到，或置信度过低）。

- `sensor_id`：
  - 对应配置文件中的 `sensor_id`，用于在控制侧区分多个雷达；

- `sensor_x_m`, `sensor_y_m`, `sensor_z_m`, `sensor_yaw_deg`：
  - 由配置文件中的 `mount` 段直接填写；
  - 便于控制系统在需要时自行进行更复杂的多雷达融合或几何重建。
