...
本节为外部控制系统开发人员提供基于 `DockingPacket` 数据进行停泊决策的参考逻辑。

#### 6.3.1 雷达配置与坐标系

感知系统支持多雷达部署，雷达位置由 `sensor_id` 标识：

| sensor_id | 位置 | 典型安装方向 | 适用场景 |
|-----------|------|--------------|----------|
| 1 (FRONT) | 船首 | 正前方 | 前靠（船头朝向码头） |
| 2 (PORT) | 左舷 | 左侧 | 侧靠左舷 |
| 3 (STARBOARD) | 右舷 | 右侧 | 侧靠右舷 |

**重要说明**：
- `distance_m` 为雷达坐标系下的检测距离
- 控制系统需根据配置文件中的 `mount` 外参将距离转换到船体坐标系
- 建议控制系统维护各雷达的外参配置（与感知系统配置一致）

#### 6.3.2 数据有效性判断

控制系统应首先判断接收到的 `DockingPacket` 是否有效：

```
函数 isPacketValid(packet):
    // 1. 检查状态码
    如果 packet.status == NOT_DETECTED (0):
        返回 无效 (未检测到目标)
    
    // 2. 检查置信度阈值（建议 >= 50%）
    如果 packet.confidence < CONFIDENCE_THRESHOLD:
        返回 低置信度警告
    
    // 3. 检查时间戳有效性（防止过期数据）
    如果 (当前时间 - packet.timestamp_ns) > TIMEOUT_NS:
        返回 数据超时
    
    // 4. 检查距离合理性
    如果 packet.distance_m < 0 或 packet.distance_m > MAX_VALID_DISTANCE:
        返回 距离异常
...
| 参数 | 推荐值 | 说明 |
|------|--------|------|
| CONFIDENCE_THRESHOLD | 50 | 置信度下限 |
| TIMEOUT_NS | 500,000,000 | 数据超时（500ms） |
| MAX_VALID_DISTANCE | 50.0 | 最大有效检测距离（米） |

#### 6.3.3 停泊条件判断逻辑

根据不同的靠泊场景，控制系统需采用不同的判断策略：

##### A. 单雷达场景（前靠或单侧靠）

```
函数 checkDockingCondition_Single(packet, targetDistance_m, tolerance_m):
    // 1. 数据有效性检查
    如果 !isPacketValid(packet):
        返回 { canDock: false, reason: "数据无效" }
    
    // 2. 距离条件判断
    distance = packet.distance_m
    
    如果 distance <= targetDistance_m + tolerance_m:
        如果 packet.confidence >= 70:  // 高置信度才允许停泊
            返回 { canDock: true, finalDistance: distance }
        否则:
            返回 { canDock: false, reason: "置信度不足，建议减速观察" }
    
    返回 { canDock: false, reason: "未达到目标距离" }
```

##### B. 多雷达场景（侧靠时同时使用左右舷雷达）

```
函数 checkDockingCondition_Multi(packets[], targetDistance_m, tolerance_m):
    // 1. 分类各雷达数据
    frontPkt = findPacketBySensorId(packets, FRONT)
    portPkt = findPacketBySensorId(packets, PORT)
    starboardPkt = findPacketBySensorId(packets, STARBOARD)
    
    // 2. 确定主传感器（靠近码头一侧）
    // 假设左舷靠泊：主传感器为 PORT
    primaryPkt = portPkt
    secondaryPkt = starboardPkt  // 辅助传感器监测对侧安全
    
    // 3. 主传感器条件检查
    如果 !isPacketValid(primaryPkt):
        返回 { canDock: false, reason: "主传感器数据无效" }
    
    // 4. 辅助传感器安全检查（可选但推荐）
    如果 isPacketValid(secondaryPkt):
        如果 secondaryPkt.distance_m < SAFETY_CLEARANCE_M:
            返回 { canDock: false, reason: "对侧检测到障碍物" }
    
    // 5. 距离与置信度综合判断
    如果 primaryPkt.distance_m <= targetDistance_m + tolerance_m:
        如果 primaryPkt.confidence >= 70:
            返回 { canDock: true, finalDistance: primaryPkt.distance_m }
    
    返回 { canDock: false, reason: "条件未满足" }
```

#### 6.3.4 无效结果处理策略

当收到无效结果（`status = NOT_DETECTED` 或低置信度）时，控制系统应采取保守策略：

| 状态 | 持续时间 | 建议动作 |
|------|----------|----------|
| `NOT_DETECTED (0)` | < 1秒 | 保持当前状态，使用上一次有效值 |
| `NOT_DETECTED (0)` | 1-3秒 | 减速并发出警告 |
| `NOT_DETECTED (0)` | > 3秒 | 紧急停止，等待人工干预 |
| `LOW_CONFIDENCE (2)` | 任意 | 降低接近速度，增加观察频率 |
| 数据超时 | > 500ms | 紧急停止，检查通信链路 |

**保守策略伪代码**：
```
函数 handleInvalidData(lastValidPacket, currentPacket, invalidDuration):
    如果 invalidDuration < 1秒:
        // 短暂丢失，使用历史数据外推
        返回 { action: "MAINTAIN", useDistance: lastValidPacket.distance_m }
    
    否则如果 invalidDuration < 3秒:
        // 中等持续时间，减速
        返回 { action: "SLOW_DOWN", speedFactor: 0.5 }
    
    否则:
        // 长时间无效，紧急停止
        返回 { action: "EMERGENCY_STOP", alert: true }
```

#### 6.3.5 安全停泊综合判断

综合考虑多传感器、置信度和时间连续性的完整判断逻辑：

```
函数 safeDockingDecision(packets[], history[], config):
    result = {
        canDock: false,
        safetyOK: true,
        confidence: 0,
    ...
    如果 max(timestamps) - min(timestamps) > MAX_TIME_SKEW_NS:
        result.warnings.append("传感器时间不同步")
    
    // 2. 主传感器（靠泊方向）状态检查
    primaryPkt = getPrimarySensor(packets, config.dockingSide)
    如果 primaryPkt.status == NOT_DETECTED:
        result.safetyOK = false
        result.warnings.append("主传感器未检测到目标")
        返回 result
    
    // 3. 置信度时间稳定性检查
    recentConfidences = getRecentConfidences(history, 5)  // 最近5帧
    avgConfidence = average(recentConfidences)
    如果 avgConfidence < config.minAvgConfidence:
        result.warnings.append("置信度不稳定")
    
    // 4. 距离趋势检查（防止距离突变）
    recentDistances = getRecentDistances(history, 5)
    如果 hasAbnormalJump(recentDistances, config.maxDistanceJump):
        result.safetyOK = false
        result.warnings.append("距离异常跳变")
        返回 result
    
    // 5. 对侧安全裕度检查（多雷达场景）
    secondaryPkt = getSecondarySensor(packets, config.dockingSide)
    如果 secondaryPkt 有效 且 secondaryPkt.distance_m < config.safetyClearance:
        result.safetyOK = false
        result.warnings.append("对侧空间不足")
        返回 result
    
    // 6. 最终停泊条件判断
    如果 result.safetyOK:
        如果 primaryPkt.distance_m <= config.targetDistance + config.tolerance:
            如果 primaryPkt.confidence >= config.minDockConfidence:
                result.canDock = true
                result.confidence = primaryPkt.confidence
...
| 参数 | 单位 | 推荐值 | 说明 |
|------|------|--------|------|
| `targetDistance` | m | 0.5 - 2.0 | 目标停泊距离，根据船型调整 |
| `tolerance` | m | 0.1 - 0.3 | 距离容差 |
| `minDockConfidence` | % | 70 | 允许停泊的最低置信度 |
| `minAvgConfidence` | % | 50 | 历史平均置信度下限 |
| `maxDistanceJump` | m | 0.5 | 最大允许的帧间距离变化 |
| `safetyClearance` | m | 3.0 | 对侧最小安全距离 |
| `maxTimeSkew` | ms | 100 | 多传感器最大时间偏差 |
